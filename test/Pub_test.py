...
from Model import *
from Pub_dataset import dataset
import datetime
import time
from torch.utils.data import DataLoader
...
model = torch.load(
   "/data4/zhuo-file/fyc_file/ATTU_KAN_Norm_copy/parameter/Public/XiongAn/CTRM_UNetW4C/CTRM_UNetW4C_lr1-3_bs1_Epoch295_2560_hybridLoss.pth",map_location="cuda:3")  # 要改为要测试的模型地址
model.to(device)
    ...
    with open(log_file, "w") as f:
        for epoch in range(1, Max_epoch + 1):
            ...
            for i, (inputs, labels) in enumerate(test_loader):  # i代表第i次迭代
                ...
                with torch.no_grad():
                    outputs = model(inputs)  # [bs, 4, 64, 64]
                # print(labels.shape, outputs.shape)

                outputs = XA_max_min_norm(outputs)
