...
class TestDataset(torch.utils.data.Dataset):
    def __init__(self, hsi_path, label_path, file_index=20, start_H=0, start_W=0):
        ...
        # 创建对应的保存文件夹
        save_folder = f"/data4/zhuo-file/fyc_file/Arti2/Sample/ours_best{file_index}_{best_start_H}_{best_start_W}"
        if not os.path.exists(save_folder):
            os.makedirs(save_folder)
        img.save(f"/data4/zhuo-file/fyc_file/Arti2/Sample/ours_best{file_index}_{best_start_H}_{best_start_W}/RGB.png")
...
for model_name, model_path in model_paths.items():
    ...
    elif model_name == "UNet_KAN":
        model = UNet_KAN()
    elif model_name == "UNet":
        model = UNet()
    model.load_state_dict(torch.load(model_path, map_location="cuda:3"))
    model.to(device)
    model.eval()
...
for model_name, model in models.items():
    ...
    with torch.no_grad():
        for i, (inputs, labels) in enumerate(test_loader):
            ...
            # 创建模型对应的保存文件夹
            save_folder = f"/data4/zhuo-file/fyc_file/Arti2/Sample/ours_best{file_index}_{best_start_H}_{best_start_W}/{model_name}"
            if not os.path.exists(save_folder):
                os.makedirs(save_folder)
...
# 新增保存平均结果到Excel的代码
excel_save_path = f"/data4/zhuo-file/fyc_file/Arti2/Sample/ours_best{file_index}_{best_start_H}_{best_start_W}/model_avg_results.xlsx"
