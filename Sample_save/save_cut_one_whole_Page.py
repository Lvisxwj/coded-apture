...
# 只保留 UNet_KAN 的参数文件路径
# model_paths = {
#     "UNet_KAN": "/data4/zhuo-file/fyc_file/Arti11/ATTU_Norm/parameter/ours/UNet_KAN_KAN/UNet_KAN_KAN_lr1-3_bs1_Epoch200_2560_hybridLoss.pth"
# }

# 只保留 UNet 的参数文件路径
model_paths = {
    "UNet": "/data4/zhuo-file/fyc_file/Arti11/ATTU_Norm/parameter/ours/UNet/UNet_lr4-4_bs8_Epoch500_2560_hybridLoss.pth"
}
...
# 绘图部分，只使用 UNet_KAN 的结果
# label_file = f"/data4/zhuo-file/fyc_file/Arti11/Sample/ours_best{file_index}_{best_start_H}_{best_start_W}/UNet_KAN/{file_index}_best_label.mat"
# model_file = f"/data4/zhuo-file/fyc_file/Arti11/Sample/ours_best{file_index}_{best_start_H}_{best_start_W}/UNet_KAN/{file_index}_best_result.mat"
# save_figure_path = f"/data4/zhuo-file/fyc_file/Arti11/Sample/ours_best{file_index}_{best_start_H}_{best_start_W}/{file_index}_compare_plot{best_start_H}_{best_start_W}.png"

label_file = f"/data4/zhuo-file/fyc_file/Arti11/Sample/ours_best{file_index}_{best_start_H}_{best_start_W}/UNet/{file_index}_best_label.mat"
model_file = f"/data4/zhuo-file/fyc_file/Arti11/Sample/ours_best{file_index}_{best_start_H}_{best_start_W}/UNet/{file_index}_best_result.mat"
save_figure_path = f"/data4/zhuo-file/fyc_file/Arti11/Sample/ours_best{file_index}_{best_start_H}_{best_start_W}/{file_index}_compare_plot{best_start_H}_{best_start_W}.png"
...
# 读取 UNet_KAN 模型的结果 mat 文件
result_data = sio.loadmat(model_file)
# 取出关键数据 (C, H, W)
...
# 绘制 UNet_KAN 模型的结果曲线
plt.plot(channels, result_avg, linestyle='--', label=model_name, linewidth=2)
...
# 这里取 > 0 的最小值来防止报错
all_avg_values = np.concatenate([[result_avg], [label_avg]])
...
with pd.ExcelWriter(excel_save_path) as writer:
    ...
    # 保存 UNet_KAN 模型的平均结果
    df = pd.DataFrame({'Vegetation index': channels, 'Average Value': result_avg})
