...
class TestDataset(torch.utils.data.Dataset):
    def __init__(self, dataset_name, hsi_path, label_path, start_H=0, start_W=0, is_Train=False):
        ...
        # 将 mask 的长宽各复制为原来的5倍
        self.mask = np.repeat(np.repeat(mask, 10, axis=0), 10, axis=1)
        ...
        height_slice = int(height * 0.8)
        width_slice = int(width * 0.3)
        
        self.hsi = self.hsi[0:height_slice, 0:width_slice, :]
        self.label = self.label[0:height_slice, 0:width_slice, :]
        self.mask_3d = self.mask_3d[0:height_slice, 0:width_slice, :]
        print(self.hsi.shape, self.label.shape, self.mask_3d.shape)
