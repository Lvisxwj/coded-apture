from .unet_parts_KAN import *

class UNet_KAN(nn.Module):
    def __init__(self, bilinear=False):
        super(UNet_KAN, self).__init__()
        ...
        self.tohyp = OutConv(64, 84)
        self.tospec = OutConv(84, 32)
    ...
    def initial_x(self, y):
        ...
        nC, step = 84, 2
        bs, row, col = y.shape
        x = torch.zeros(bs, nC, row, col-(84-1)*2).cuda(0).float()
        for i in range(nC):
    ...
    def forward(self, x):
        ...
        x_hyp = self.tohyp(x1_1)
        
        x1_1 = self.tospec(x_hyp)
        return x_hyp, x1_1
